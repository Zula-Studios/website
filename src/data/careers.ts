/** Stellenanzeigen — Übersicht + Detailseiten unter /karriere/[slug] */

export type CareerJob = {
  slug: string;
  title: string;
  /** Kürzerer Titel für Karten */
  cardTitle: string;
  tag: string;
  iconName: 'shop' | 'code';
  /** Teaser für Übersichtskarten */
  teaser: string;
  /** 3 kurze Punkte auf der Übersicht */
...
export const CAREERS_DATE_POSTED = '2026-04-17';
export const CAREERS_VALID_THROUGH = '2027-04-17';
...
export const careerJobs: CareerJob[] = [
  {
    ...
    iconName: 'shop',
    teaser:
      'Von Theme & Liquid bis Apps und APIs — du machst Shopify-Projekte technisch rund.',
    highlights: [
    ...
    intro:
      'Du bringst Shops zum Laufen: von Theme-Anpassungen und Sections über Shopify Functions bis zu privaten Apps und API-Integrationen. Bei uns arbeitest du an echten Kundenprojekten — ohne Agentur-Bingo, mit klarem Fokus auf Qualität.',
    responsibilities: [
  ...
  },
  {
    ...
    iconName: 'code',
    teaser:
      'Backend, APIs und Integrationen — wenn der Standard-Shop an Grenzen stößt.',
    highlights: [
    ...
    intro:
      'Du entwickelst maßgeschneiderte Lösungen jenseits des Standard-Shops: Backend-Services, Schnittstellen und Integrationen für anspruchsvolle E-Commerce-Projekte. Du bist die technische Stütze, wenn Kunden mehr brauchen als Theme und App Store.',
    responsibilities: [
...
/** Labels für das Bewerbungsformular (value → Anzeige / E-Mail). */
export function getStelleLabel(slug: string): string {
  if (slug === 'initiativ') return 'Initiativbewerbung';
  return getCareerBySlug(slug)?.title ?? slug;
}

export const bewerbungStelleOptions: { value: string; label: string }[] = [
